...
PITCH_LOW = 21
PITCH_HIGH = 108  # inclusive


def midi_to_88_pr(midi_path, sr=22050, hop_length=512):
    pm = pretty_midi.PrettyMIDI(str(midi_path))
    fs = float(sr) / float(hop_length)
    pr128 = pm.get_piano_roll(fs=fs)  # shape (128, T)
    # extract 21..108 (inclusive) -> indices 21..108
    pr88 = pr128[PITCH_LOW:PITCH_HIGH + 1, :]
    # binarize: note present if velocity>0
    pr88_bin = (pr88 > 0).astype(np.uint8)
    return pr88_bin
...
def align_time_frames(pr, target_T):
    ...
    if T > target_T:
        return pr[:, :target_T]
    # T < target_T -> pad with zeros on right
...
def main():
    ...
    parser.add_argument('--midi-dir', type=Path, default=Path('Audio Files/2015'), help='Input folder containing MIDI files (default: Audio Files/2015)')
    parser.add_argument('--mel-dir', type=Path, default=Path('mels'), help='Folder containing mel .npy files (default: mels)')
